...
void fmi2TransferConnectionValues(connection c, fmi2_import_t ** fmus, fmi2_import_variable_list_t** variables){
    int ci, found=0, k, i, l;

    // Temp for transfering values
    fmi2_base_type_enu_t type;
    fmi2_value_reference_t vrFrom[1];
    fmi2_value_reference_t vrTo[1];
    fmi2_real_t rr[2];
    fmi2_boolean_t bb[2];
    fmi2_integer_t ii[2];
    fmi2_string_t ss[2];
    ...
    // Get variable list of both FMU participating in the connection
    fmi2_import_variable_list_t* varsFrom = variables[fmuFrom]; // fmi2_import_get_variable_list(fmus[fmuFrom]);
    fmi2_import_variable_list_t* varsTo =   variables[fmuTo];   // fmi2_import_get_variable_list(fmus[fmuTo]);
    int numFrom = fmi2_import_get_variable_list_size(varsFrom);
    int numTo   = fmi2_import_get_variable_list_size(varsTo);

    for (k=0; numFrom; k++) {
        fmi2_import_variable_t* v = fmi2_import_get_variable(varsFrom,k);
        if(!v) break;
        //vrFrom[0] = fmi2_import_get_variable_vr(v);
        fmi2_base_type_enu_t typeFrom = fmi2_import_get_variable_base_type(v);

        // Now find the input variable
        for (l=0; !found && l<numTo; l++) {

            fmi2_import_variable_t* vTo = fmi2_import_get_variable(varsTo,l);
            if(!vTo) break;
            //vrTo[0] = fmi2_import_get_variable_vr(vTo);
            fmi2_base_type_enu_t typeTo = fmi2_import_get_variable_base_type(vTo);
            ...
            if(typeFrom == typeTo){
                ...
                case fmi2_base_type_real:
                    fmi2_import_get_real(fmus[fmuFrom], vrFrom, 1, rr);
                    fmi2_import_set_real(fmus[fmuTo],   vrTo,   1, rr);
                    break;
                case fmi2_base_type_int:
                case fmi2_base_type_enum:
                    fmi2_import_get_integer(fmus[fmuFrom], vrFrom, 1, ii);
                    fmi2_import_set_integer(fmus[fmuTo],   vrTo,   1, ii);
                    break;
                case fmi2_base_type_bool:
                    fmi2_import_get_boolean(fmus[fmuFrom], vrFrom, 1, bb);
                    fmi2_import_set_boolean(fmus[fmuTo],   vrTo,   1, bb);
                    break;
                case fmi2_base_type_str:
                    fmi2_import_get_string(fmus[fmuFrom], vrFrom, 1, ss);
                    fmi2_import_set_string(fmus[fmuTo],   vrTo,   1, ss);
                    break;
                default:
                    printf("Could not determine type of value reference %d in FMU %d. Continuing without connection value transfer...\n", (int)vrFrom[0], (int)fmuFrom);
                    break;
                }

                found = 1;
            } else {
                printf("Connection between FMU %d (value ref %d) and %d (value ref %d) had incompatible data types!\n",(int)fmuFrom,(int)vrFrom[0],(int)fmuTo,(int)vrTo[0]);
            }
        }
    }
}
